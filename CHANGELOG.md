...
### Added
- **Regression guard — reject a change that breaks the test suite (`--test-cmd`).**
  Opt-in. Given the project's own test command (e.g. `--test-cmd "pytest -q"`), the
  engine runs it **inside the same srt sandbox as the build worker** — once on a
  baseline worktree at `HEAD`, then on each graded worktree — and fails any task
